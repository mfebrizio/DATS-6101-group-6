...
```{r line, results='markup'}

# use summ_by_year

ggplot(summ_by_year, aes(x = year)) + 
  expand_limits(y=c(0,5)) + 
  geom_line(aes(y = wmean_mh_days, color = 'poor MH days'), size=1.5) +
  geom_point(aes(y = wmean_mh_days)) + 
  scale_y_continuous(
  ...
  geom_line(aes(y = wmean_mh_rate, color = 'MH distress rate'), size=1.5) +
  geom_point(aes(y = wmean_mh_rate)) 


# Most basic line chart
p1 <- ggplot(summ_by_year, aes(x=year)) +
  geom_line(aes(y = wmean_mh_days)) + 
  expand_limits(y=c(0,5)) +
  labs(title = "Poor Mental Health Days", x = "Report Year", y = "Avg for past 30 days")

p2 <- ggplot(summ_by_year, aes(x=year)) +
  geom_line(aes(y = wmean_mh_rate)) + 
  expand_limits(y=c(0,.25)) +
  labs(title = "Frequent Mental Distress Rate", x = "Report Year", y = "Percentage of adults")
