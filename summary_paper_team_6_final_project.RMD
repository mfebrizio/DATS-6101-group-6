...
Secondly, is a strong time influence on the models, as indicated by how the year variable is consistently one of the most important variables in the regression tree and random forest models. This makes it difficult to determine if economic inequality has a substantial impact on mental health or if these variable are just correlated overtime. Further research may want to focus on data within a single time period. 

Third, the models could be tuned and constructed better to provide more reliable predictions and lower RMSE values. For exampled, the regression trees could be reduced in size by choosing a smaller max depth and more aggressive complexity parameter.  
