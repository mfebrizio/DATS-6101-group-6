...
Our paper is structured as follows. First, we open by describing our data set and summarizing the exploratory data analysis (EDA) that informed our statistical modeling. Next, we carried out feature selection for the linear regression models to ascertain which variables produced the best adjusted R-squared value. Then, we evaluated a series of linear regression models, using both dependent variables, and compared the results. Finally, we built regression tree models and random forests as a secondary method of supervised learning.

Our results suggest that mental health and economic inequality exhibit statistically significant correlations. Additionally, random forests were determined to be the best predicting models and variables ranked the consistently highest in feature importance include year, region, median_inc, and child_poverty. Finally, there are many limitations that negatively impact the reliability of this project's analysis including the use of self-reported mental health data, the use of only adjusted R-squared for feature selection, the strong influence of the year variable on the models, and poorly tuned models for accurate and precise predictions of the target variables.     
...
The results from the linear models for both dependent variables – `mental_health_days` and `mental_distress_rate` – are similar. Every model has a statistically significant coefficient on `inequality`, and the coefficients for the majority of regions are statistically significant too. These general trends remained after adding more independent variables, to make our models more robust.
...
However, when `mental_distress_rate` is the dependent variable, the strength of the association is generally weaker than for the models with `mental_health_days` as the dependent variable. This could be partly due to how the variables are measured. For example, `mental_health_days` will be a number greater than zero (and usually > 1), while `mental_distress_rate` is always a percentage ranging from 0 to 1 (and typically < 0.25). Additionally, these differences could also suggest that the relationship between mental health and inequality also depends on the measure of mental health used.
...
The regression tree model analyzes the relationship between number of poor mental health days versus all the economic inequality variables included in our dataset. Additionally, region and year were included as an explanatory variables to observe the regional and temporal affects on the dependent variable. In other words, the model regresses mental_health_days against inequality, median_inc, hs_grad, college, unempl, child_poverty, single_parent, severe_housing, food_index, mh_providers, pop_provider_ratio, region, and year.
...
The summary table of the regression tree indicates the order of variable importance for each variable within the regression. Again, the top three most important variables in order of importance is year, child_poverty, and median_inc.
...
The summary table of the regression tree indicates the order of variable importance for each variable within the regression. Interestingly, the top three most important variables in order of importance is child_poverty, year, and median_inc.
...
The summary table of the regression tree indicates the order of variable importance for each variable within the regression. Again, the top three most important variables in order of importance is child_poverty, median_inc, and year.
...
The top three most important variables in order of importance by %IncMSE are year, median_inc, and region. The top three most important variables in order of importance by IncNudePurity are year, child_poverty, and median_inc. The RMSE value of this random forest is 0.327.
...
The EDA and hypothesis testing conducted in the Midterm Project suggested that the relationship between mental health and economic inequality differs by region. Linear modeling indicates that the relationship between mental health and economic inequality is statistically significant. However, the strength of association between mental and economic inequality does vary by region. Both measures of mental health exhibited similar relationships, but specifically the number of poor mental health days variable displayed more pronounced associations. The Midwest region had the strongest association between each mental health dependent variable. The South had the weakest association between each mental health dependent variable. 
...
Finally, the RMSE model comparisons revealed that the random forest models were the best predicting models due to their low RMSE values relative to the other models. The worst predicting models were the regression trees due to their large RMSE values relative to the other models. The reason for why the regression trees are the worse predicting models is because they are over fitting the data and are unable to make accurate predictions given data different from the train set. The reasons why the random forest models likely performed better at predicting include the model's ability to effectively deal with high dimensional data, it's robustness to outliers, it's indifference to non-linear data, it's ability to handle unbalanced data, it's low bias, and it's moderate variance. Overall, random forest models should probably be used to make reliable predictions regarding the prevalence of poor mental health based on various economic variables. 
...
Secondly, there is a strong time influence on the models, as indicated by the year variable consistently being ranked as one of the top most important variables in the regression tree and random forest models. This makes it difficult to determine if economic inequality has a substantial impact on mental health or if these variable are just correlated overtime. Further research may want to focus on data within a single time period. The strong influence of the year variable likely reduces reliability and accuracy of the models. 

Third, the models could be constructed and tuned better to provide more reliable predictions and lower RMSE values. For exampled, the regression trees could be reduced in size by choosing a smaller max depth and a more aggressive complexity parameter. Additionally, a better model selection tool could have been chosen to select for better linear models. Adjusted R-squared was initially chosen because the measure has a penalty for adding additional explanatory variables that do not improve the fit of the model. Adjusted R-squared also allows for comparisons across multiple models with different numbers of independent variables. For future research, model selection should be conducted by comparing AIC or BIC values respectively. AIC and BIC allows for better model selection by providing a measure for model performance that accounts for model complexity. The AIC or BIC value reflects how well the model fits the data and penalizes the model in proportion to its number of parameters. AIC tends to favor larger models whereas BIC tends to favor parsimonious models.

Overall, the limitations above call into question the explanatory power of these models. These limitations make the results of the models unreliable. Interestingly, a consistent result that appears across the models is the importance of the region variable, indicating that poor mental health differs regionally in the United States. However, it is unclear if these regional differences are due to economic inequality, differences in regional and social attitudes discussing mental health, data collection biases, or some other unknown influence. Even though there are limitations with this project further analysis is needed on the relationship between mental health and economic inequality. 

Overall, this project suggests that our initial hypothesis, mental health and economic inequality are related, was largely on the right track. However, our results also indicate that other predictors might be more important than income inequality when considering the effects on mental health.
