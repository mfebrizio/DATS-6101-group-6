...
Summary paper, 25%. Due on May 8 Sunday. Write a roughly 10-page (no more than 5000
words, charts do not count) summary of the research and EDA process of your project. The summary should be prepared in R-markdown, and knitted into HTML. You may take
some of the work in part III (such as graphs and results) to include here. They can overlap. This summary is to-be presented to your boss, your client, or to-be submitted for publication in journals. Potential area of topics to address in this summary may include:
...
Our paper is structured as follows. First, we open by describing our data set and summarizing the exploratory data analysis (EDA) that informed our statistical modeling. Next, we carried out feature selection for the linear regression models to ascertain XX. Then, we evaluated a series of linear regression models, using both dependent variables, and compared the results. Finally, we built regression tree models and random forests as a secondary method of supervised learning.

Our results suggest XX.
...
```
In model with best 10 selected feeature, adjusted r2 has declined significantly to 0.461.
...
```
As we can see, at 24, we are getting the highest R2 and we have plotted the graph below. 
...
As we can see, at 23, we are getting the maximum adjusted R2 and we have plotted the graph below. 
...
As we can see, at 22, we are getting the lowest CP and we have plotted the graph below.
...
As we can see, at 22, we are getting the minimum BIC. 
...
As we can see, at 24, we are getting the highest adjusted R2 and we have plotted the graph below. 
...
As we can see, at 23, we are getting the lowest CP and we have plotted the graph below. 
...
As we can see, at 19, we are getting the lowest BIC and we have plotted the graph below. 
...
The regression tree model analyzes the relationship between number of poor mental health days versus all the economic inequality variables included in our dataset. Additionally, region and year variables were included as an explanatory variables to observe the regional and time impact on the dependent variable. In other words, the model regresses mental_health_days against inequality, median_inc, hs_grad, college, unempl, child_poverty, single_parent, severe_housing, food_index, mh_providers, pop_provider_ratio, region, and year.

The initial regression tree model below has a max depth of 8 and a complexity penalty (cp), or complexity parameter, of 0.004. This is to allow for a large regression tree in order to produce a model that can best fit the data. However, it must be acknowledged that a larger tree will run the risk of over fitting the data making the model worse at predicting the target variable (i.e. the dependent variable). 

The following regression tree has 17 leaf nodes. 

The summary table of the regression tree indicates the order of variable importance for each variable within the regression. Interestingly, the top three most important variables in order of importance is year, child_poverty, and median_inc.
...
Designing models that accurately represent the relationships between mental health and economic inequality is a difficult but important task. The growing body of academic literature outlines that economic inequality is worsening within the United States and  health disparities increasingly occur along socioeconomic gradients. As health inequity continues to grow, mental health is likely to be impacted by the widening economic divide. 

The EDA and hypothesis testing conducted in the Midterm Project suggested that the relationship the relationship between mental health and economic inequality differs by region. Linear modeling indicates that the relationship between mental health and economic inequality is statistically significant. However, the strength of association between mental and economic inequality does vary by region. Both measures of mental health exhibited similar relationships, but specifically the number of poor mental health days variable displayed more pronounced associations. The Midwest region had the strongest association between each mental health dependent variable. The South had the weakest association between each mental health dependent variable. 

The regression tree models constructed for both mental health dependent variables indicated that the top three most important variables are year, percentage of child poverty, and median income. Interestingly, for both mental health dependent variables the income inequality variable was considered to be the eighth most important variable. Although this finding does not outright contradict the notion that income inequality plays an important role in determining poor mental health, it must be noted that the income inequality variable does rank lower than expected. This is surprising given that the income inequality variable is the most direct measure of economic inequality as it is a ratio of household income between the 80th and 20th percentiles. One possible explanation for this is that the income inequality variable does not have a lot of variation within the dataset suggesting it does not have a lot of explanatory power for the variation in two mental health dependent variables. 

The random forest models constructed produced somewhat similar results regarding variable importance. However, depending on the measure of variable importance, either %IncMSE and IncNodePurity, different variables were ranked within the top three most important. For both random forest models, the top three most important variables measured by %IncMSE include year, median income, and region. For both random forest models, the top three most important variables measured by IncNodePurity include year, percentage of child poverty, and median income. Again, income inequality ranked lower than expected for both random forest models. The income inequality variable was ranked as the eleventh most important variable by both measures of variable importance. 

Finally, the RMSE model comparisons revealed that the random forest models were the best predicting models due to their low RMSE values relative to the other models. The worst predicting models were the regression tree due to their large RMSE values relative to the other models. The reason for why the regression trees are the worse predicting models is because they are over fitting the data and are unable to make accurate predictions given different data from the train set. The reasons why the random forest models likely performed better at predicting include the model's ability to effectively deal with high dimensional data, it's robustness to outliers, it's indifference to non-linear data, it's ability to handle unbalanced data, it's low bias, and moderate variance. Overall, random forest models should probably be used to make reliable predictions regarding the prevalence of poor mental health based on various economic variables. 

# Limitations

Although these conclusions are interesting and insightful, there are severe limitations that curtain the reliability and accuracy. First, the mental health target variables rely on self-reported mental health data. This data cannot be validated with medical records. Further research could be done by using a binary mental health variable in order to generate a confusion matrix, accuracy score, recall score, precision score, and ROC-AUC graphs. 

Secondly, is a strong time influence on the models, as indicated by how the year variable is consistently one of the most important variables in the regression tree and random forest models. This makes it difficult to determine if economic inequality has a substantial impact on mental health or if these variable are just correlated overtime. Further research may want to focus on data within a single time period. The strong influence of the year variable likely curtails the reliability and accuracy of these models. 

Third, the models could be tuned and constructed better to provide more reliable predictions and lower RMSE values. For exampled, the regression trees could be reduced in size by choosing a smaller max depth and more aggressive complexity parameter. Additionally, a better model selection tool could have been chosen to select for better linear models. Adjusted R-squared was initially chosen because the measure has a penalty for adding additional explanatory variables that do not improve the model the fit of the model. Adjusted R-squared also allows for comparisons across models with different numbers of independent variables. For future research model selection should be conducted by comparing AIC and BIC values. AIC and BIC allows for better model selection by providing a measure for model performance that accounts for model complexity. The AIC/BIC value reflects how well the model fits the data and penalizes the model in proportion to its number of parameters. AIC tends to favor larger models whereas BIC tends to favor parsimonious models. 

Overall, the limitations above call into question the explanatory power of these models. These limitations make the results of the models unreliable. Despite these limitations a consistent result that appears across the model is the importance of the region variable, indicating that poor mental health differs United States varies regional. However, it is unclear if these regional differences are due to economic inequality, differences in regional and social attitudes discussing mental health, or data collection biases. Even though there are limitations with this project further analysis is needed on the relationship between mental health and economic inequality. 
